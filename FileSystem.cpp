...
void FileSystem::Update() {
	...
	inodeTable.UpdateInodeTable();//更新 Inode 表
...
Inode* FileSystem::IAlloc() {
	...
	if (superBlock.s_ninode <= 0) {
		...
		for (int i = 0; i < superBlock.s_isize; i++) {
			...
			//检查缓冲区中每个外存Inode的i_mode是否为0，非0表明已被占用
...
void FileSystem::IFree(int number) {
	...
	//将编号为 number 的 Inode 添加到超级块直接管理的空闲 Inode 索引表中
	superBlock.s_inode[superBlock.s_ninode++] = number;
	//设置超级块被修改的标志。
	superBlock.s_fmod = 1;
}

//从超级块的空闲块索引表中分配一个空闲磁盘块
Buf* FileSystem::Alloc() {
	...
	if (superBlock.s_nfree <= 0) { //直接管理的空闲盘块为空
		...
		//将该盘块的头404个字节读入超级块的s_nfree和s_free[100]
...
//将编号为 blkno 的磁盘块释放并加入到超级块的空闲块索引表中
void FileSystem::Free(int blkno) {
	...
	if (superBlock.s_nfree <= 0) {
		...
		superBlock.s_free[0] = 0;//空闲盘块链结束标志
	}

	//如果SuperBlock直接管理的空闲磁盘块号的栈已满
	//则将s_nfree和s_free表的内容写到当前正在释放的盘块的头404字节
