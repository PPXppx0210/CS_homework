...
void FileManager::FormatSystem() {
	...
	//初始化数据块
	//使用 DirectoryEntry 结构填充文件系统的根目录和其他几个重要目录的目录项
...
void FileManager::Creat() {
	...
	if (pInode == NULL) {
		pInode = this->MakNode(1);//创建新Inode
		if (pInode == NULL) {
			return;
		}
		this->Open1(pInode, 2);
...
void FileManager::Open1(Inode* pInode, int trf) {
	...
	OpenFileTable& openFIleTable = Kernel::Instance().GetOpenFileTable();
	InodeTable& inodeTable = Kernel::Instance().GetInodeTable();
	if (trf == 1) { //如果要创建的文件已经存在
		pInode->ITrunc();
	}
	//从打开文件表分配File结构,并相应地设置其属性
...
void FileManager::Close() {
	State& state = Kernel::Instance().GetState();
	OpenFileTable& openFileTable = Kernel::Instance().GetOpenFileTable();
	int fd = state.arg[0];//从系统状态中获取文件描述符
	//获取打开文件控制块File结构
...
void FileManager::Seek() {
	...
	//如果state.arg[2]在3-5之间
	//意义同0-2，但长度单位由字节变为512字节
	if (state.arg[2] > 2) {
		offset = offset << 9;
		state.arg[2] -= 3;
	}
...
Inode* FileManager::NameI(char (*func)(), enum DirectorySearchMode mode) {
	...
	//如果路径名为空且操作模式不是 OPEN，则返回错误
	if ('\0' == curchar && mode != FileManager::OPEN) {
		inodeTable.IPut(pInode);//释放当前Inode
		return NULL;
	}
	...
	while (true) {
		...
		//内层循环部分对于state.dbuf[]中的路径名分量，逐个搜寻匹配的目录项
		...
		while (true) {
			...
			for (int i = 0; i < sizeof(DirectoryEntry) / sizeof(int); i++) {
				*((int*)&state.dent + i) = *(src + i);//将目录项内容拷贝到 state.den
			}
			//更新偏移量，减少剩余计数
		...
		//跳到这里说明当前路径分量匹配成功，还需匹配下一路径分量，直到'\0'结束
...
Inode* FileManager::MakNode(unsigned int mode) {
	...
	//如果创建的是目录，则还会创建该目录的"."和".."目录项，并将其写入磁盘
	if (mode == 2) {
...
void FileManager::rm() {
	...
	if (pInode->i_mode == 1) {
		cout << "无法对文件执行rmdir指令" << endl;
		return;
	}
	//如果是目录，则递归地删除其内部的所有文件和子目录
